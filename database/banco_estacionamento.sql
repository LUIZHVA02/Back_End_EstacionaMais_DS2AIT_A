...
insert into
    tbl_usuarios(nome, email, telefone, endereco, cpf, senha)
values
    (
        "Vitor Hugo",
        ...
        "124.629.461-09",
        aes_encrypt("03/06/2024", "chave")
    );
    
/* Decriptação de senhas
select nome, CAST(AES_DECRYPT(senha,'chave') as char) from tbl_administradores WHERE id > 0; 
*/
...
insert into
    tbl_administradores(nome, email, telefone, endereco, cpf, senha)
values
    (
        "Raica Martinez",
...
insert into
    tbl_pagamentos(forma_pagamento)
values
    ("Debito"),
    ("Dinheiro"),
    ("Crédito"),
...
insert into
    tbl_vagas(sigla_vaga, tipo_vaga, status_vaga)
values
